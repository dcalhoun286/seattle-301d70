...
function displayPics(){
  ...
  // the variable rando was more locally scoped than where console.log was being invoked
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} votes in ${allProducts[i].views}`);
    }
  }
